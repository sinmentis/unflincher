...
def test_lang_cookie_name():
    assert LANG_COOKIE_NAME == "unflincher_lang"
...
def test_get_current_language_reads_supported_cookie():
    assert get_current_language(_fake_request("unflincher_lang=zh-Hans")) == "zh-Hans"


def test_get_current_language_falls_back_for_unsupported_cookie_value():
    assert get_current_language(_fake_request("unflincher_lang=klingon")) == "en"
